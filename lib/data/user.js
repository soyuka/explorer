...
const debug = require('debug')('explorer:data:user')
...
const ReasonsError = require('../errors/ReasonsError.js')
...
function User(user, crypt) {
  ...
  let reasons = this.isValid(user)

  if(reasons !== true) {
    throw new ReasonsError(reasons)
  }
...
User.prototype.getCookie = function getCookie() {
  ...
  properties.forEach(key => key === 'password' ? null : cookie[key] = this[key])

  cookie.home = p.normalize(cookie.home)
  return cookie
}
...
User.prototype.sanitize = function sanitize(user) {
  for(let i in schema) {
    ...
    } else if(schema[i].type == 'string' && schema[i].default === undefined && user[i] !== undefined) {
      if(user[i].trim() === '') 
        delete user[i]
    }
  }
...
User.prototype.isValid = function isValid(user) {
  ...
  let valid = Object.keys(schema)
  .filter(e => {
    if(!user.hasOwnProperty(e)) {
      reasons.push(e + ' is required') 
      return true
    } 

    if(/:/g.test(user[e])) {
      reasons.push(e + ' can not contain the character ":"')
      return true
    }
  })
