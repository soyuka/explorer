...
var _statJs2 = _interopRequireDefault(_statJs);
...
for (var i in args) {
  try {
    ...
    if (!_title) {
      _title = p.basename(args[i]);
    }

    if (plugins[_title]) {
      throw new ReferenceError('Plugin ' + _title + ' is already registered');
    }

    plugins[_title] = tmp.job(ipc, new _statJs2['default'](_title));
  } catch (e) {
    console.error('Could not require plugin (export a function) %s', title);
    console.error(e.stack);
    if (plugins[title]) delete plugins[title];
  }
}
