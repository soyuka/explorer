...
class User {
  ...
  constructor(user, crypt = true) {
    this.properties = ['username', 'password', 'home', 'key', 'admin', 'readonly']

    user.admin = this.valueToIntegerBool(user.admin)
    user.readonly = this.valueToIntegerBool(user.readonly)
  ...
  update(user, ignore) {
    ...
    for(var i in user) {
      if(~ignore.indexOf(i) || !~this.properties.indexOf(i) || typeof user[i] === 'undefined')
        continue;
