...
let debug = require('debug')('explorer:users')
...
class User {
  ...
  isValid(user) {
    let valid = Object.keys(schema).filter(e => !user.hasOwnProperty(e) || /:/g.test(user[e]))
  ...
  toString() {
    ...
    for(let p in schema) {
      if(schema[p].type == 'buffer') {
        var b = util.isArray(this[p]) ? this[p].join(eol) : this[p]
