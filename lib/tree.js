...
var recursiveReaddir = function(root, memory) {
  ...
  .map(function(f) {
    ...
    .then(function(stat) {
      if(stat.isDirectory()) {
        return recursiveReaddir(path, memory) 
      }
 ...
 */
var paths = function(path, recursive) {
  if(recursive == true) {
    return recursiveReaddir(path, {root: path})
    .map(function(e) {
...
var tree = function(path, options) {
  ...
  //this is used only for nativeSearch
  ...
  var calcDirectorySize = function(f) {
    ...
    var resolver = function() {
      if(cache) {
        return Promise.all([
          cache.time.put(hash, ''+f.mtime),
          cache.size.put(hash, ''+f.size)
        ])
        .then(function() {
          return f
        })
  ...
  return paths(path, options.recursive)
