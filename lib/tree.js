...
var directorySize = function directorySize(file, root, options) {
  ...
  return fs.statAsync(filePath).then(function (stat) {
    ...
    if (err.code == 'EACCES') console.error('No file access (check rights)', filePath);else console.error('No file entry (file does not exist ?!)', filePath);
...
var paths = function paths(path) {
  if (typeof path === 'string') return fs.readdirAsync(path);else if (_util2['default'].isArray(path)) return _bluebird2['default'].all(path);
...
var tree = function tree(path, options) {
  ...
  }, concurrency).filter(options.searchFilter ? options.searchFilter : function (e) {
    return e;
  }).map(options.sort == 'size' ? calcDirectorySize : function (e) {
    return e;
