...
var tree = function tree(path, options) {
  ...
  return paths(path).filter(_utilsJs.noDotFiles, concurrency).map(function (f) {
  ...
  }, concurrency).call('sort', options.sortMethod || function () {
    return;
    ...
    var breadcrumb = options.search ? buildBreadcrumb(options.root) : buildBreadcrumb(options.root, path);
