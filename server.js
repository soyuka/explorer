...
var _libPluginsJs = require('./lib/plugins.js');
...
module.exports = function (config) {
  ...
  app.use(_bodyParser2['default'].urlencoded({ extended: false, limit: config.upload.maxSize }));
  app.use(_bodyParser2['default'].json({ limit: config.upload.maxSize }));
  ...
  app.set('plugins', (0, _libPluginsJs.registerPlugins)(config));
  ...
  (0, _expressSession2['default'])({ secret: config.session_secret || 'MEOW', resave: false, saveUninitialized: false }), (0, _connectFlash2['default'])(), _express2['default']['static']('client')]));
  ...
  app.use((0, _libUtilsJs.parallelMiddlewares)([middlewares.format(app), middlewares.notify, middlewares.optionsCookie, middlewares.registerHooks(app)]));
